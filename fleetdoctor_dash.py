...
class LighthouseError(RuntimeError):
    """A Lighthouse API or connection error."""
...
def load_env_file() -> Path | None:
    """Load .env values without evaluating shell code."""
    ...
    for path in candidates:
        ...
        for line_number, raw_line in enumerate(
            ...
            if "=" not in line:
                print(
                    f"WARNING: ignored {path}:{line_number}; expected NAME=value.",
                    file=sys.stderr,
                )
                continue
...
class LighthouseClient:
    """Minimal read-only client used by the standalone dashboard."""
    ...
    def __init__(
        ...
        self.base_url = f"{lighthouse.rstrip('/')}/api/{api_version.strip('/')}"
    ...
    def _request(
        ...
        try:
            ...
            raise LighthouseError(
                f"HTTP {exc.code} at {endpoint}: {detail}"
            ) from exc
        except urllib.error.URLError as exc:
            raise LighthouseError(f"Could not connect to {url}: {exc.reason}") from exc
    ...
    def login(self, username: str, password: str) -> None:
        ...
        if str(result.get("state") or "").lower() == "failed":
            raise LighthouseError("Lighthouse rejected the credentials")
        token = result.get("session")
        if not token:
            raise LighthouseError(
                f"Lighthouse did not create a session "
    ...
    def get_all_nodes(self) -> list[dict[str, Any]]:
        ...
        while True:
            ...
            total_pages = int((result.get("meta") or {}).get("total_pages", 1) or 1)
            if page >= total_pages:
                return nodes
            page += 1
...
def progress_bar(value: int, total: int, width: int) -> str:
    """Return a compact terminal bar without claiming unavailable metrics."""
    width = max(3, width)
    ratio = 0.0 if total <= 0 else min(1.0, max(0.0, value / total))
    filled = round(ratio * width)
    return "#" * filled + "." * (width - filled)


def sparkline(values: list[int], width: int) -> str:
    """Render an in-memory health history; oldest sample is on the left."""
    width = max(3, width)
    points = values[-width:]
    symbols = ".:-=+*#@"
    graph = "".join(symbols[min(7, max(0, int(point)))] for point in points)
    return "." * (width - len(graph)) + graph
...
class FleetDashboard:
    def __init__(
        ...
        self.health_history: dict[str, list[int]] = {}
    ...
    def refresh_data(self) -> None:
        ...
        try:
            ...
            for node in views:
                node_id = str(node["id"])
                history = self.health_history.setdefault(node_id, [])
                history.append(health_score.get(str(node["severity"]), 2))
                del history[:-40]
                node["health_history"] = history.copy()
            views.sort(
                key=lambda node: (
    ...
    def draw(self) -> None:
        ...
        if height < 24 or width < 90:
            safe_addstr(
                ...
                1,
                2,
                "OOB Fleet Doctor requires a terminal of at least 90 x 24.",
            ...
            safe_addstr(
                ...
                3,
                2,
                f"Current terminal: {width} x {height}",
    ...
    def draw_summary(self, width: int) -> None:
        ...
        bar_x = 15
        bar_width = max(10, width - bar_x - 3)
        safe_addstr(
            self.screen,
            6,
            2,
        ...
        segments = [
            (counts["HEALTHY"], PAIR_GREEN),
            (counts["WARNING"], PAIR_YELLOW),
            (counts["CRITICAL"], PAIR_RED),
            (counts["UNKNOWN"], PAIR_MAGENTA),
        ]
        assigned = 0
        for index, (count, pair) in enumerate(segments):
            if index == len(segments) - 1:
                segment_width = max(0, bar_width - assigned)
            else:
                segment_width = round((count / total) * bar_width)
                assigned += segment_width
            if segment_width:
                safe_addstr(
                    self.screen,
                    6,
    ...
    def draw_node_card(
        ...
        lines = [
            ...
            (
                "Health trend",
                sparkline(
                    list(node.get("health_history") or []),
                    max(8, inner_width - 21),
                ),
                color,
            ),
            (
                "Port config",
                f"{progress_bar(int(node['configured_ports']), int(node['total_ports']), max(8, inner_width - 30))} "
                f"{node['configured_ports']}/{node['total_ports']}",
                PAIR_GREEN,
            ),
            (
                "Sessions",
                f"{progress_bar(int(node['active_sessions']), max(1, int(node['configured_ports'])), max(8, inner_width - 30))} "
                f"{node['active_sessions']}",
                PAIR_YELLOW if int(node["active_sessions"]) else PAIR_WHITE,
            ),
